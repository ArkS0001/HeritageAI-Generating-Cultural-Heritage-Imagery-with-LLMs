dekho pre trained models, mask aur 1-2 cheeje khud bnani padegi, ek default values bna dege sabki

yeh rha model->
https://huggingface.co/drengskapur/Pix2Vox-A-ShapeNet.pth/tree/main
...
st.title("3D Reconstruction from 2D Views")
st.markdown('This application takes 2D imaged views of objects and converts it into 3D objects', unsafe_allow_html=False)
st.markdown('*Step 1:* Upload an image and process it using Pix2Vox architecture trained on ShapeNet DataSet to convert 2D images to 3D voxel objects', unsafe_allow_html=False)
...
if not os.path.isfile(MODEL_FILENAME):
    ...
    MODEL_URL = 'https://2d-to-3d-pretrained-model.s3-us-west-2.amazonaws.com/Pix2Vox-A-ShapeNet.pth'
    resp = requests.get(MODEL_URL, stream=True)
    with open(MODEL_FILENAME, 'wb') as file_desc:
        for chunk in resp.iter_content(chunk_size=5000000):
            file_desc.write(chunk)
...
def generate_data():
    ...
    if torch.cuda.is_available():
        ...
        map_location = torch.device('cpu')
        checkpoint = torch.load(MODEL_FILENAME, map_location=map_location)
    
    cfg.CONST.WEIGHTS = MODEL_FILENAME
    generated_volume, rendering_images = test_net(cfg, output_dir='./output')
    vol = generated_volume.reshape(32,32,32)
    return vol
...
if img_file_buffer is not None:
    ...
    st.image(image, caption='Uploaded Image', use_column_width=True)
    cv2.imwrite("load_images/0.jpg", image)
